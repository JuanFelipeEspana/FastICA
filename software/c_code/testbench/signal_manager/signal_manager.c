...
void signal_create(signal* p_new_signal, char signal_name[], char signal_type[], uint32_t signal_bits_size, uint8_t order, uint32_t signal_initial_value)
{
  ...
  array_copy(signal_name, (p_new_signal->name), string_size(signal_name), 1);
  array_copy(signal_type, (p_new_signal->type), string_size(signal_type), 1);
...
{
  uint32_t statement = (signal->current_statement);
...
{  
  ...
  for(uint32_t i = 0; i < (signal->current_statement); i++)
  {
