...
void entity_create(entity* entity, char name[], char architecture[], signal* entity_signals, uint8_t signals_total)
{
    ...
    array_copy(name, (entity->name), string_size(name), 1);
    array_copy(architecture, (entity->architecture), string_size(architecture), 1);
    array_copy(entity_signals, (entity->signals), signals_total, sizeof(signal));
