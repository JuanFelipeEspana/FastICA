...
int main(){
  ...
  //printMatrix(mixedData, N_SAMPLES, N);
    ...
    double *a=(double *)(z_array);
      while( iter_count<max_iter && convergence_difference > tol ){
        ...
        for (int i=0;i<number_samples;i++){
            ...
            //printCMakeLists.txtf("g_vector antes %f %f %f\n",*(g_vector),*(g_vector+1),*(g_vector+2));
        ...
        convergencCMakeLists.txte_difference=fabs(fabs(dotProduct_unrolled(wp,w_temp,m))-1);
        copy(wp,w_temp,m);
        if(p == 0)
          printVector(wp, m);
        //printf("El iter es %d\n",iter_count);
...
double **randomMixing(double **originalSources, int rows, int cols){
  double **mixMatrix;
  double **mixedMatrix;
  double vector[] = {0.5, -0.2, -0.3, 0.5, -0.5, 0.6, 0.9, -0.1, 0.1};
  mixMatrix = foldVector(vector, 3);
  mixedMatrix = asymmetricMatrixProduct(originalSources, rows, cols, mixMatrix, cols, cols);
...
double **whiten(double **originalMatrix, int rows, int cols){
  ...
  originalMatrixTranspose = transposeMatrix(centeredMatrix, rows, cols); //originalMatrixTranspose = originalMatrix'
  ...
  printMatrix(kTranspose, cols, cols);
