...
void random_Matrix_generate(float_matrix* matrix,uint32_t rows, uint32_t cols, double max_range){
    for (uint32_t i=0;i<rows;i++){
        for(uint32_t j=0;j<cols;j++){
            float random_number = ((float)random_number_generate( (65355) )) * (float)pow(2.0, max_range - 16.0);
...
{   
    ...
    random_Matrix_generate(&matrix_a, matrix_a_rows, matrix_dim_common, 16);
    matrix_a.max_exponent = 0;
    random_Matrix_generate(&matrix_b, matrix_dim_common, matrix_b_cols, 8);
