#include <matrix_multipĺier_arch.h>
...
matrix_multiplier_addr* matrix_multiplier_get_virtual_address(uint32_t matrix_multiplier_avalon_address)
{
    ...
    matrix_multiplier_address = (matrix_multiplier_addr*) (avalon_lw_get_virtual_address() + matrix_multiplier_avalon_address);
...
void matrix_multiplier_multiply(float_matrix* matrix_a, float_matrix* matrix_b, float_matrix* matrix_c, matrix_multiplier* multiplier)
{
    ...
    uint8_t matrix_multiplier_address_size = ((multiplier->mac_size) + (multiplier->memory_size)) - 2;
    ...
    *((multiplier->address) + TOTAL_SIZE) = (matrix_A->rows)*(matrix_A->cols);
    ...
    //MATRIX B REGISTERS
    *((multiplier->address) + DECIMAL) = 0;
    *((multiplier->address) + TOTAL_SIZE) = (matrix_B->rows)*(matrix_B->cols);
    //MATRIX B VALUES
    memcpy((void* ) ((multiplier->address) + (0x1 << matrix_multiplier_address_size - 2)), (void*) (matrix_B->address), (matrix_B->rows)*(matrix_B->cols)*sizeof(float));

    //WAIT TILL DATA IS READY
    while(*((multiplier->address) + DATA_READY) == 0);

    //COPY RESULTS FROM ARCHITECTURE TO MATRIX IN MAIN MEMORY
    memcpy((void* ) (matrix_c->address), (void* ) ((multiplier->address) + (0x3 << matrix_multiplier_size)), ((matrix_a->rows)*(matrix_b->cols))*sizeof(float));
}

void matrix_multiplier_configuration_print(matrix_multiplier_addr* multiplier)
{
